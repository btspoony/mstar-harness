...
**维护**：**@project-manager** 在重大里程碑后更新（例如 QC 波次结束、批量归档 resolved、版本封板前）。可选在 **`metadata.notes`** 记一条「已刷新 `tech_debt_summary`」。
...
- **`total_open` / `by_*`**：应与当前 **`metadata.residual_findings`** 中所有 **open**（未归档）条目的数量与严重度**大致一致**；若有意的「跨 plan 合并视角」导致计数口径不同，在 `metadata.notes` 或 `cross_cutting` 中说明。
...
**热文件中的瘦 `Done` 行**（团队采纳本做法后）：

- 保留 **规范顶层键**（`id`、`title`、`file`、`status`、`done_at`、`notes` 等）；`status` 为 `Done` 时，将 `metadata` **缩减**为导航所需，通常保留：
  - **`archived_record`**：相对 `{PLAN_DIR}` 的路径，例如 `archived/plans/<plan-id>.json`
  - 若该 `plan-id` 在 **`metadata.residual_findings`** 中仍有 **open** 行，可保留一行 **`residual_summary`**（语义见上文 **`residual_summary`（可选）** 小节）
- 一旦快照已写入，可从热行中省略臃肿字段（`gates`、`qc_status`、`tests`、`commits`、长 `description`/`scope` 等）；完整内容以快照文件为准。
- **`notes`** 保持简短（合并结果、分支、指向 `reports/<plan-id>/`）；**不要**重复已存在于 QC 报告中的大段原文。
...
在合并关闭计划相关工作的分支或打开 PR 前，**@project-manager**（或项目约定角色）宜核对：`plans[].status`、`plans[].metadata.gates`（若使用）、**`metadata.residual_findings`**、**`metadata.tech_debt_summary`**（若使用）、**`metadata.notes`**（若用其记录程序里程碑）与审查结论、CI/测试结果一致。**SSOT 与事实不一致时，不宜视为可合并**，应先修正。
...
- 团队若用 **`metadata.notes`** 作程序时间线：重大合并或批量归档后**未**追加条目，导致后续 agent 缺少上下文。
