...
- `clarify`（歧义清理）
  ...
  - **澄清怎么问（按宿主）**：需要补信息或做取舍时，**若宿主提供结构化提问能力**（例如 OpenCode 的 **`question` 工具**），优先使用（标题 + 题干 + 选项；可附自定义文本作答）。**本仓库默认倾向**：能拆成可选项或互斥决策时，用结构化方式收敛；选项少而精。**兜底**：需大段叙述、开放式访谈、或强约束下选项会误导时，再用自由正文提问。OpenCode 用户须自行保证 `opencode.json` 中 **`permission.question` 为 `allow`（或未 deny）**；其他宿主见 `host-cursor.md`。agent 不得擅自改全局密钥类配置。
- `plan`（实现设计）
...
**与模型配置**：在支持按角色切模型的宿主上（例如 OpenCode 的 `opencode.json` 为不同 subagent 配置 `model`），与「类别 → 强项模型」同向；PM 可在 `Why this agent` 中点名类别与选型理由。

## 内置 `@explore` 能力边界（强制）

在支持 **`@explore`** 的宿主上（如 OpenCode），它是**只读**、偏快速的代码库导航 subagent，用于**缩短找文件/符号/粗粒度结构**的时间。无该能力时由执行方用 `glob`/`grep`/`read` 或宿主等价工具达成同一目标；调度语义不变。
