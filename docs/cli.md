...
Keywords are free text; recognized tokens:

| Token | Meaning | Default |
|---|---|---|
| `quick` / `deep` | Effort level: hotspot-only / full-repo coverage (subagent fan-out 0–1 vs ≤8) | `standard` |
| `<category>` | Category focus — run recon, then that category only: `security`, `perf`, `tests`, `tech-debt`, `deps`, `dx`, `docs` | all nine |
...
| `simplify` | DEBT-focused deep pass: dead / duplicated / speculative / over-built / added-then-removed / hand-rolled-where-a-dependency-exists surfaces, proved or rejected via consumer classification; findings use category `DEBT`; tiny-real items land in "considered and rejected", never inline TODOs | — |
