...
Consumer plans default to **`.mstar/`**. Process artifacts (`plans/`, `iterations/`, `status.json`, `sdd/`, …) are gitignored; tracked results: `{HARNESS_DIR}/AGENTS.md`, `knowledge/`, `specs/`. Specs resolve `.mstar/specs/` → `docs/specs/` → repo-root `specs/`. Repos with a non-default harness root can declare it in a gitignored **`.mstarc`** (`[config] harness_dir=<dir>` — honored above probing). Details → `mstar-plan-conventions`.
