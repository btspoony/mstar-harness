...
- **Status:** Frozen. Authoritative semantics for plan-level workflow delivery. Corpus surfaces cite it pointer-level (plan `20260916-plan-lifecycle-contracts` T2); engine/CLI seams implement against it (plan `20260916-plan-lifecycle-engine-gates`). Where corpus prose and this contract disagree on lifecycle semantics, this contract is the wording authority until formally amended.
- **Baseline:** canonical repository sources at HEAD `d54ddd41aa0c14344d4f7173dc5ac5b0ce6fb2b8`. Engine line references describe that SHA (verified by the §1.6 architect pass and re-confirmed by this contract's author).
- **Evidence baseline:** `projects/_default/references/plan-workflow-lifecycle-gap-survey.md` (same SHA). Its scope-decision table is adopted verbatim in §7 with locked answers.
...
- **Owner:** PM (`_default`). Authored by `architect`, 2026-09-16.

## Foundational distinctions (survey-verbatim)
...
## 6. Engine seam inventory (implemented by `20260916-plan-lifecycle-engine-gates`)

Three named seams. Per-seam acceptance checks are drawn from the survey's required verification scenarios. Nothing in this section is implemented by this contract's owning plan.
...
**Explicit deferral.** Mid-lifecycle advancement-gate breadth is deferred: per-stage engine gates across recall, prepare/lock, execute, compound disposition, and PR submission are not in the engine-gates round. `evaluatePhaseGate` stays iteration-shaped. The process obligations for those stages are carried by corpus pointers to this contract; only registration admission (S2) and terminal evidence (S3) are wired into code, plus the S1 producer. This deferral is explicit and deliberate, not an omission.

## 7. Scope decisions (locked)

Adopted verbatim from the survey §"Scope decisions to lock" (direction and reason columns unchanged); the locked answers are binding. There are no open decisions in this table.

| Decision | Locked answer | Locked direction (survey, verbatim) | Reason (survey, verbatim) |
|---|---|---|---|
...
> Amendment 2026-09-16 (PM reconciliation of task-1-review Minor #1 / qc1 F-002): the §7 row-3 direction cell originally paraphrased the survey ("the standalone plan's"); it has been restored to survey-verbatim wording ("its"). This note records that the earlier in-place fix preceded this amendment entry; the locked answer is unchanged.
