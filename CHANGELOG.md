...
- Updated `skills/mstar-host/references/omp.md`, `_shared/host-role-binding-core.md` (Kimi/ZCode vs omp host classes), `parallel-dispatch.md`, and `mstar-host` skill description; aligned INSTALL / `docs/cli.md`. Dropped the README “Host notes / 宿主说明” aside so the Use section stays entry-only.
