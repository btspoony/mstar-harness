...
5. `docs/agents/plan-convention.md` — **`{HARNESS_DIR}`**（默认 `.agents/`，SSOT 与时间线等）与 **`{PLAN_DIR}`**（默认 `{HARNESS_DIR}/plans/`，主 plan 与 `reports/`）的发现与初始化、**`{HARNESS_DIR}/status.json`**（**open** residual、`metadata.residual_findings` 中空 `plan-id` 键须删除、可选 `detail_doc` 指向 **`{PLAN_DIR}/residuals/<plan-id>/`** 散文、可选 `metadata.tech_debt_summary` 技术债一览）、**`{HARNESS_DIR}/archived/residuals/<plan-id>.json`**、可选 **`{HARNESS_DIR}/archived/plans/<plan-id>.json`**（Done 行冷快照与热文件**极简**行）、可选 **`{HARNESS_DIR}/notes.json`**（程序时间线，减轻 **`{HARNESS_DIR}/status.json`** 体积）、`{PLAN_DIR}/reports/<plan-id>/`、**`{HARNESS_DIR}/knowledge/`** 与 `docs/` 分工、**已提交文档可到达性**（禁仓库外路径、禁仅本机路径）、Git 策略、合并前 SSOT 与事实一致。
...
- 执行 Superpowers `writing-plans` 时，计划文件须落在 `plan-convention.md` 解析到的 **`{PLAN_DIR}`**；**`{HARNESS_DIR}/status.json`**、**`{HARNESS_DIR}/notes.json`**、**`{HARNESS_DIR}/archived/`**、**`{HARNESS_DIR}/knowledge/`** 等初始化与 SSOT 规则见同文档；不得默认写入 `docs/superpowers/plans/`。
