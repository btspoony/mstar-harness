...
| 项目计划 | 项目 cwd 下 `.agents/plans/` > `.plans/` > `plans/`（按优先级发现） | 相对路径 `{PLAN_DIR}/status.json` |
...
- 计划管理约定：`~/.config/opencode/docs/agents/plan-convention.md`
- 项目状态与计划：`{PLAN_DIR}/status.json`（项目 cwd，目录按优先级发现）
- 计划详情与决策日志：`{PLAN_DIR}/*.md`（项目 cwd）
...
6. `~/.config/opencode/docs/agents/plan-convention.md` — 计划目录约定
...
1. 澄清意图和验收标准。
2. 在项目 plan 目录中创建或更新计划条目（目录发现规则见 `~/.config/opencode/docs/agents/plan-convention.md`）。
...
- 行为变更必须同步更新测试。
- 拒绝未记录的破坏性变更。
- **业务 Git 仓库与功能分支**：凡对当前 cwd 下**业务/应用 Git 仓库**产生可合并的代码或运行相关配置改动，默认在**功能分支**（非 `main`/`master` 等默认保护分支）上完成；在默认分支上直接改代码须由 `@project-manager` 在 Assignment 中**显式写明** `Branch policy` 例外（例如约定热修）。细则见 `~/.config/opencode/docs/agents/harness-loop.md` 中的「Git 功能分支门禁」。
- 将反复出现的失败编码为规则或检查。
