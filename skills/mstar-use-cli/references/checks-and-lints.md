...
This file indexes the check commands — the ones skill engine-check callouts cite and the ones a gate or a review runs before trusting an artifact. Almost every row is a thin transport over an engine validator: it reads a document, applies that validator's contract, and reports violations. Two rows are not checks at all — `mstar audit scaffold` and `mstar audit promote` write files and register a workflow — and are indexed here only so the audit family is not mistaken for a read-only one (see their row). Use it to answer three questions: which command checks this artifact, which skill owns the rules behind it, and what its exit code means.
...
| `mstar audit scaffold` / `mstar audit promote` | **not a check — both write.** `scaffold` creates an `audit-<date>/` plan directory (numbered plan files + README index) from a findings file, at `--dir` or `./audit-<date>` from cwd; `promote` writes a workflow snapshot (type plan, Todo rows, the declared delivery kind + its evidence) and then **registers the workflow in `{HARNESS_DIR}/status.json`** — a guarded registration entry point, not a review-seat check. `promote` declares its delivery kind at registration and refuses without `--delivery-kind`; run either only under the authorization its owning skill names | `mstar-audit` | `0` wrote / registered · `1` failure (for `promote`, also an unresolvable harness dir) · `2` usage |
