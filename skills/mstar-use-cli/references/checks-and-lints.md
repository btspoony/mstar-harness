...
This file indexes the check commands — the ones skill engine-check callouts cite and the ones a gate or a review runs before trusting an artifact. The validator and lint rows are a thin transport over an engine validator: each reads a document, applies that validator's contract, and reports violations. The table is not all validators, though — it also carries an informational rollup, the issue-store lifecycle writes, phase and push gates, and PR-review arithmetic and path contracts — and two rows are not checks at all: `mstar audit scaffold` and `mstar audit promote` write files and register a workflow, indexed here only so the audit family is not mistaken for a read-only one (see their row). Use it to answer three questions: which command checks this artifact, which skill owns the rules behind it, and what its exit code means.
...
| `mstar status tech-debt` | the issue store's open-issue rollup ({HARNESS_DIR}/store.db): total open, by severity, by project. Informational: the store is the source of truth, so there is no stored-summary drift check. A missing, corrupt or staged store refuses — never an empty rollup | `mstar-project-governance` | `0` rollup (zero when the store has no open issues) · `1` project dir or store not readable / resolution failed |
| `mstar status findings-cleanup` | enforces a plan's cleanup mode over the OPEN issues linked to that plan in the issue store, keyed by plan id; an explicit mode wins, otherwise the permissive default. Plans without linked issues pass trivially. A missing, corrupt or staged store refuses instead of passing as no findings | `mstar-artifacts` | `0` no open-issue violations · `1` one row per violating open issue |
| the `store` group — `migrate` | the staged issue/catalog migration: a read-only preview manifest by default (byte digests, source-set digest, mappings, unresolved rows, catalog conflicts, retirement sections; writes no DB), or the explicit reviewed `--apply` — one transaction, persistent ID mapping, source-drift refusal. An applied store stays **staged**: ordinary mutations are refused until activation | `mstar-project-governance` (capture/source mapping), `mstar-conventions` (store authority) | `0` preview or applied · `1` source drift / unresolved mapping / IO refusal · `2` usage |
| the `store` group — `backup` / `activate` / `retire` | the store lifecycle: a quiesced SQLite-consistent `VACUUM INTO` backup verified by reopening it; the activation barrier (final apply receipt + compatible-consumer attestation + backup, one transaction flipping authority and epoch); retirement of the exact reviewed legacy registers and index sections under a resumable ledger. **Not read-only rows**: run them only under the recorded authorization, never from a review seat | `mstar-conventions` (store vs execution JSON); `references/status-and-registers.md` § Store lifecycle | `0` done · `1` refusal (stale receipt, missing attestation, legacy write detected) · `2` usage |
...
| `mstar status archive-residuals` | nothing — removed twice over. It exits `1` and names the replacement: findings are issues in the store now, closed through the plan-scoped or unscoped issue disposition. The residual register it used to touch is migration history | — (points at `mstar-project-governance`) | `1` always |
...
| `mstar worktree cleanup` | `worktree cleanup --workflow <id> [--harness <path>] [--apply] [--remote] [--worktree <path>] [--all-workflows] [--verbose] [--ignore-unreadable-snapshots]`: guarded worktree and branch cleanup, default candidates from the selected workflow's recorded claims (`--all-workflows` full sweep; `--worktree` also narrows branch candidates by exact owner; `--verbose` per-pair ancestry diagnostics); dry-run by default printing one verdict row per candidate; ownership / merge-evidence / refusal / apply-order semantics → `mstar-branch-worktree`「Worktree / branch cleanup」 | `mstar-branch-worktree` (call sites: `mstar-iteration` phase-2 same-round + phase-6 post-merge close) | `0` valid dry run or successful removals · `1` probe or mutation failure · `2` usage |
...
Two rows write state under a lock — the lifecycle close and the migration — and the audit pair writes outside any lock: `mstar audit scaffold` creates a plan directory and `mstar audit promote` registers a workflow. The retired register verbs from the issue/catalog cutover are deliberately absent here: they parse, refuse and name their issue replacement, so they are not a transport for anything. Read-only rows are what make this table usable from a review seat, so before running a row from one, read the row itself: never invoke the two audit writers, the store-lifecycle rows or the scoped migration from a read-only seat.
...
- an informational command (the tech-debt rollup) reports a projection of the issue store; the store stays the source of truth;
