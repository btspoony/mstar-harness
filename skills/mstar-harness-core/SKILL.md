...
|------|------|-------------------|
| **全部** | 本 skill | — |
| **`@project-manager`** | 本 skill | `mstar-dispatch-gates`、`mstar-phase-gates`、`mstar-plan-conventions`、`mstar-superpowers-align`、`mstar-roles`；派 QC 前 `mstar-review-qc`；并行/审查 `mstar-branch-worktree`；维护 status/residual `mstar-status-residuals`；InReview/QC 波次 `mstar-plan-artifacts`。**不**读 `mstar-coding-behavior` |
| **实现/审查/QA/运维** | 本 skill + `mstar-coding-behavior` + 角色 ref | 有 git 写：`mstar-branch-worktree`；有 plan 路径：`mstar-plan-conventions`（路径符号节）；QC/QA：`mstar-review-qc`；改 status/residual：`mstar-status-residuals` |
...
`status.json` 字段与 residual → **`mstar-status-residuals`**。
...
| `mstar-status-residuals` | `status.json`、residual severity 与生命周期 |
...
| residual 只写 plan 不写 SSOT | `mstar-status-residuals` |
| 角色文件塞流程长文 | 用专题 skill |
