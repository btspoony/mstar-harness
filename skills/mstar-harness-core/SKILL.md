...
`specify → clarify → plan` → `plan(locked) → tasks → implement` → QC 三审 + QA → Done → 知识结晶（`mstar-compound`）。阶段细则 → **`mstar-phase-gates`**；知识结晶 → **`mstar-compound`**。
...
|------|------|-------------------|
| **全部** | 本 skill | — |
| **`@project-manager`** | 本 skill | `mstar-dispatch-gates`、`mstar-phase-gates`、`mstar-plan-conventions`、`mstar-superpowers-align`、`mstar-roles`；派 QC 前 `mstar-review-qc`；并行/审查 `mstar-branch-worktree`；plan/status/reports `mstar-plan-artifacts`；UI 类 plan Prepare 阶段 `mstar-design-md`（DESIGN.md 门禁）；Done 后知识结晶 `mstar-compound`；战略性工作 `mstar-strategy`。**不**读 `mstar-coding-behavior` |
...
| `mstar-compound` | 知识结晶 —— 已解决问题→结构化知识文档，双轨（Bug/Knowledge），重叠检测，可发现性检查，CONCEPTS.md 协同 |
| `mstar-compound-refresh` | 知识维护 —— 审查/更新/合并/删除 `{KNOWLEDGE_DIR}` 文档，保持准确性 |
