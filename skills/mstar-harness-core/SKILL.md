...
| `mstar-audit` | Variant carrier：common core（hard rules、recon、vet、variant dispatch）+ `references/codebase-audit.md`（full-audit 变体：9 类别 fan-out、effort、scope variants、Phase 4、output、handoff）+ `references/pr-review.md`（`pr` 变体）；`audit-playbook` + `finding-format` + `plan-quality-bar` |
