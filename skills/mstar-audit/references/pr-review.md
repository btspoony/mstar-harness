...
- **Register the rest as audit todos** — before the review starts, register every unprocessed PR in `{PROJECT_DIR}/_default/residuals.json` (project-less reviews always use `_default`):
  ...
  - Registration is a **single-writer** operation — only the command's main agent writes the register, before review dispatch; concurrent same-date sessions must read the current register first and bump the batch key after reading (same-host, serialized), never blind-overwrite.
