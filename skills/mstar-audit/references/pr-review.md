...
- **Register the rest as audit todos** — before the review starts, register every unprocessed PR in `{PROJECT_DIR}/_default/residuals.json` (project-less reviews always use `_default`):
  ...
  - Registration of deferred PRs MUST run inside a **same-host exclusive write lock** on the register path — the engine status-write lockdir pattern (`<register dir>/.status-write.lockdir/`, atomic-mkdir; the shared-lifecycle lockdir pattern used by engine register writers, see `mstar-artifacts` / `{KNOWLEDGE_DIR}/architecture-patterns/shared-lifecycle-lockdir-pattern.md`) — covering the full **read-check-replace-verify** sequence: read the current register, pick the next free same-date batch key, write, then `validateProjectRegister` — all inside the lock. Concurrent same-date sessions serialize on the lock; never blind-overwrite. **Acquisition**: `mkdir <register dir>/.status-write.lockdir/` is atomic; immediately after it succeeds, write `holder.pid` (the acquiring process id) into the lockdir as part of the same acquisition step — a lockdir **without** `holder.pid` is by-construction stale (acquisition never completed, e.g. a crash between the mkdir and the pid write). **Guaranteed release**: the writer captured the lockdir's `(dev, ino)` at acquisition. On completion (every posting/failure/exit branch): re-stat the lockdir; remove `holder.pid` and `rmdir` the lockdir ONLY when `(dev, ino)` is unchanged (it is still the writer's own lock — `rmdir` on a non-empty directory fails, so never attempt it while `holder.pid` is inside). If the path was replaced by another owner in between, do NOT remove it (never delete another writer's lockdir) and leave the replacement lock intact. Never leave the lockdir behind when it is still the writer's own. **Stale-lock recovery** (on lock-acquisition timeout): registration is a bounded, sub-second critical section — a lockdir held for longer than a staleness threshold (e.g. mtime older than 10 minutes) is by-construction a dead writer's leak (no live writer holds the lock across that window). Capture the lockdir's `(dev, ino)` at the age check; if older than the threshold, re-stat immediately before removal and remove the lockdir ONLY if `(dev, ino)` is unchanged — if the path was recreated by a fresh owner in between, abort the recovery (do not remove a fresh lock) and retry acquisition normally. Never remove a lockdir whose identity changed; never decide by PID liveness alone — a reused PID would make an unrelated process look like the owner (pid-file hazard). `holder.pid` inside the lockdir is crash diagnosis only, matching the engine `withStatusWriteLock`. A second timeout after a recovery attempt → halt and report `Blocked`. The full protocol (acquisition, ownership, release, recovery) is `{KNOWLEDGE_DIR}/architecture-patterns/shared-lifecycle-lockdir-pattern.md`.
