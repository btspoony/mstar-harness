...
- **Register the rest as audit todos** — before the review starts, register every unprocessed PR in `{PROJECT_DIR}/<project-id>/residuals.json` (project-less reviews use `_default`) via the engine-backed CLI, one `--entry` per deferred PR:
  ...
  - Lock-semantics delta vs. the old hand-rolled protocol: crash-leaked lockdirs are auto-reclaimed by the engine lock once their holder process is provably dead; otherwise the writer waits out the bounded wait and reports `Blocked`.
