...
- **Register the rest as audit todos** — before the review starts, register every unprocessed PR in `{PROJECT_DIR}/<project-id>/residuals.json` (project-less reviews use `_default`) via the engine-backed CLI, one `--entry` per deferred PR:
  ...
  - Lock-semantics delta vs. the old hand-rolled protocol: crash-leaked lockdirs are auto-reclaimed by the engine lock once provably stale — the holder pid is dead (signal-0 probe → `ESRCH`), or the lockdir carries no readable `holder.pid`, which the engine's atomic publish (pid file written before the lockdir is renamed into place) makes impossible for a live holder, so a live writer's lock is never reclaimed. Otherwise the writer waits out the bounded wait and reports `Blocked`.
