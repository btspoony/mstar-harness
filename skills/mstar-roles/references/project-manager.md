...
- **PM never runs as a subagent.** `project-manager` ships **no agent shell** — the host adapter excludes it from the `task` dispatch surface, and it is never a `task` target on any host. A **leaf** that receives `pm` / `project-manager` / `iteration-drive` wording (role name, handoff prose, `QA gate` field, routing table, multi-track narrative) is **not** being promoted: the leaf stays inside its own task, reports the mismatch, and does **not** dispatch, invoke, or absorb PM scope (`mstar-dispatch-gates` § role boundary / anti-recursion).
