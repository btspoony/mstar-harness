...
- **`failed` / `stopped` is not a current route.** No supported installed JSON/CLI verb writes a terminal `failed`/`stopped` status with a recorded reason today (the DB lifecycle carries a terminal branch, but that is not a supported installed writer). Until one exists, a workflow that must become terminal without a successful completion evidence record is a named blocker for its owner — reported as such, never hand-edited and never closed as `completed`.
