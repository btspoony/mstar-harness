...
**Machine-checked lines** (`mstar qc validate-report <report.md>`): the frontmatter fields `report_kind` / `reviewer` / `reviewer_index` / `plan_id` / `verdict` / `generated_at`; the body `**Verdict**:` line must agree with the frontmatter `verdict` (trailing prose is fine); each `## Summary` count must equal the top-level entries under the matching `## Findings` severity section; `Approve` is refused while Critical/Warning counts are non-zero; `Unconfirmed` is refused while the report declares `Truncated coverage:`. A cap stop is not a channel failure — keep the earned verdict and write the truncation line.
