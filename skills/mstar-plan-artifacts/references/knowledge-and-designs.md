...
5. **结晶（Compound）**：PM 在 plan Done 后触发 `mstar-compound` 将问题解决经验沉淀为结构化知识文档（`{KNOWLEDGE_DIR}/<category>/<slug>.md`，YAML frontmatter + 双轨模板）。定期维护由 `mstar-compound-refresh` 执行（审查/更新/合并/删除过期文档）。详见 **`mstar-compound`** 与 **`mstar-compound-refresh`**。
