...
- **Plugin hooks** (`hooks/hooks.json`, ZCode-only surface): **SessionStart** injects harness-workspace context ({HARNESS_DIR} + `status.json` summary) when a harness workspace is detected — a partial substitute for the missing session auto-load; **PreToolUse (Bash)** enforces the `mstar-branch-worktree` gates deterministically (no direct commit on the default protected branch → `MSTAR_ALLOW_DEFAULT_BRANCH_COMMIT=1` escape; no bare `git push --force` → use `--force-with-lease`; disable with `MSTAR_BRANCH_GUARD=off`); **PreToolUse (Write|Edit)** runs the engine-backed coordination-write gate: under opt-in `.mstarc`/compass hard enforcement it blocks writes to harness coordination documents (`status.json`, workflow snapshots), and it **unconditionally** refuses the two authority invariants in every mode — a direct write to the issue/catalog store (`{HARNESS_DIR}/store.db`, `-wal`, `-shm`) and a write to a retired project register while the store is the active authority — plus an unreadable authority (below-floor runtime, missing `node:sqlite`, corrupt or busy store), which fails closed with the engine's refusal code. Soft-enforced and unrelated writes still pass silently. Disable the whole hook with `MSTAR_WRITE_GATE=off`. Silent no-op outside git repos / harness workspaces. Skill text stays authoritative; these hooks only backstop it.
...
- **Runtime**: the plugin **hooks** are Node scripts — run them under **Node >=24.18.0** (in-process native `node:sqlite`; the store capability is acquired lazily, so a status-only write never opens the database). Harness **commands** invoked as CLI go through the installed binary's own entrypoint: its Bun shebang needs **Bun >=1.4.0**, an explicit `node <bundle>` needs Node >=24.18.0 — one floor per entrypoint, never both. Below-floor or missing-capability refuses actionably; no transport or JSON fallback.
- **Upgrade / reload**: update **morning-star-harness** from the `mstar-local` marketplace (see § Plugin/CLI version alignment) or re-run `init --target zcode`, then reload the plugin. Materialized sources under `~/.zcode/cli/plugins/` are refreshed by that install, not by editing this checkout.
