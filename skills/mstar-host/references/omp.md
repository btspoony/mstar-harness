...
**Managed bootstrap — `mstar_coordinator {operation:"bind", workflowId}`.** The only advertised managed route, registered by `extensions/model-handoff.js` and independent of the `modelHandoff` setting:

- The input is exactly `operation` and `workflowId`; the registered schema is strict, and a session id, root, caller role, authority flag or credential path has no home in it — the adapter refuses any extra key by name (`forbidden-field`). The adapter takes the **native** session id from the host session manager and the canonical control harness root from the host cwd, then calls the engine directly, preserving the existing residency, registration-committed and duplicate-holder gates — the workflow must be running with a committed catalog registration, and `coordination.duplicate-holder` names the current owner.
- Refusals: `identity-missing` (this host session has no native id — the engine never generates one), `leaf-session`, `scoped-plan-route` (that route restores an existing binding and never bootstraps one), `harness-not-found`, `invalid-input`, plus the engine's own codes passed through unchanged. Success is `bound`, with the engine-issued session on the result.
...
- A plain local operator binds without the tool: `mstar plan bind --coordinator --workflow <id> --session-id <id>`, the id stated explicitly and never generated (omitted → `coordination.identity-missing`). The inherited `MSTAR_HOST_SESSION_ID` stays a declared local input form for a **plan/assignment** bind only — it never authorizes a coordinator bootstrap.
- The same entry registers the recovery pair. `{operation:"show-recovery", workflowId}` reads the recorded owner session id, both byte versions and the Prepare verdict without writing (`recovery-allowed` / `recovery-blocked`). `{operation:"recover", workflowId, expectedSnapshotVersion, expectedCompassVersion, operationId, reason, authorizationRef, stoppedSessionIds}` replaces a binding its prior owner can no longer authenticate: the **new** identity is derived from this session, and the **prior** holder and its envelope are resolved from the engine's stored binding, never from the call — an empty stop assertion refuses `unauthorized`. Success is `recovered`, or `replayed` for an exact retry of an accepted operation; a changed request, a stale version or a superseded binding refuses. Its projection carries the workflow id, the old/new **public** session ids, the operation id, the replay flag and both versions only — never envelope bytes, an envelope path or any credential. This is the JSON/Prepare-only repair, deliberately narrower than the active-store session recovery, which keeps its full execution token and stop attestation.

No comparison is relaxed. An **absent** association (a host session with no id) and a **foreign** one (the id belongs to another session) still refuse: readiness keeps failing its `binding-invalid` code, the start-authority refusals (`task-session`, `plan-pm-session`, `coordinator-elsewhere`) keep their codes, and an id the engine cannot use as the envelope's file name is refused with `coordination.invalid-session-id` before any write. One host session may hold both sessions a workflow needs — a coordinator session and a plan-pm session — both carrying the same host-derived `session_id`; the engine distinguishes the two envelopes by the role-scoped file name (`<role>-<session-id>.json`). `--resume` never re-identifies and refuses `--session-id` as a usage error.
