...
> Amendment 2026-09-17 (iteration `iter-20260917-lifecycle-and-review-seats` Prepare, recorded at PM lock; authoring decision frozen in `.mstar/specs/plan-lifecycle-standalone-completion.md` § "Legacy repair"): **legacy delivery-source correction.**
>
...
>
> This extends §4a (registration is an authorized domain operation) with a bounded repair for pre-existing snapshots only, and leaves §5 (failure and abandonment) and the Binding negatives above untouched. It is recorded here so the runtime mirror (`skills/mstar-artifacts/references/plan-workflow-lifecycle-contract.md`) can be updated in Phase 2 against a frozen source rather than an in-flight draft. The two historical snapshots this applies to, and the adoption sequence, are named in the plan `20260917-plan-lifecycle-unblock` Task 4.
