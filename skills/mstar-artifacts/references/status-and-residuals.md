...
- The recovery projection is limited to public facts — the workflow id, the old/new public session ids, the operation id, the replay status and both byte versions. An envelope **path** is coordinator-owned transport, never part of a public diagnostic, a leaf assignment or an audit record.
...
- Global coordinator binding takes the snapshot lock but increments no row revision — it changes only top `coordination.coordinator` and `updated_at`. The audited JSON Prepare recovery replaces that same binding, refreshes `updated_at` and appends its own immutable `coordination.identity_recoveries` record (→ § Prepare coordinator recovery below).
...
- **Projection**: the workflow id, the old/new **public** session ids, the operation id, the replay status and both byte versions. Envelope bytes, an envelope path and credentials are never projected (→ Session envelopes above). Verb flags and exit codes → **`mstar-use-cli`** `references/plan-and-workflow.md` § Prepare coordinator recovery.
