...
export function assessShadowRun(input: FrozenShadowEvidence): ShadowRunAssessment {
  ...
  if (input.requiredUnitIds.some((id) => !unitIds.has(id))) throw new Error("jev.receipt-accounting-incomplete");
  const unaccountedInventory = input.requiredUnitIds.length > 0 && inventoryUnitIds.some((id) => !unitIds.has(id));
  ...
  const workUnitIds = input.requiredUnitIds.length === 0 ? [] : [...new Set([...inventoryUnitIds, ...input.requiredUnitIds])];
  const incompleteUnits = workUnitIds.length - completedUnits - blockedUnits - cancelledUnits;
  const failures = [...(lifecycleInvalid ? ["probe-lifecycle-invalid"] : []), ...(unaccountedInventory || input.requiredUnitIds.some((id) => !inventoryUnitIds.includes(id)) ? ["jev.receipt-accounting-incomplete"] : []), ...(blockedUnits > 0 ? ["jev.required-work-unit-blocked"] : []), ...(input.failures ?? [])].slice(0, 32).map((failure) => typeof failure === "string" ? failure.replace(/[^a-zA-Z0-9.-]/g, "-").slice(0, 96) : "jev.failure-invalid");
  if (completedUnits + blockedUnits + cancelledUnits + incompleteUnits !== workUnitIds.length) throw new Error("jev.receipt-accounting-invalid");
  return Object.freeze({ schema: "mstar.shadow-assessment/v1", runId: baseline.runId, evidenceClass: input.evidenceClass, qualification: input.evidenceClass === "component" ? "component-only" : "synthetic-offline", w5: false, baselineFrozen: true, childEvents: Object.freeze(childEvents), receipts: Object.freeze(receipts), metrics: Object.freeze({ childEvents: childEvents.length, workUnits: workUnitIds.length, completedUnits, blockedUnits, cancelledUnits, incompleteUnits, elapsedMs: input.elapsedMs, childOutputBytes: input.childOutputBytes ?? 0 }), failures: Object.freeze(failures) });
}
