...
import { validatePack, validatePilot, type JudgmentPilot, type ReviewDecisionPack, NATIVE_MODEL } from "./contracts.js";
...
export type FrozenBaseline = Readonly<{ schema: "mstar.shadow-baseline/v1"; runId: string; frozenAt: string; inventory: unknown; inventorySha256: string; seatOutputsSha256: string; originalConsumptionSha256: string; finalReportSha256: string; baselineSha256: string }>;
export type WorkUnitReceipt = Readonly<{ schema: "mstar.shadow-receipt/v1"; runId: string; unitId: string; packId: string; packSha256: string; scopeSha256: string; model: typeof NATIVE_MODEL; owner: "original"; disposition: "completed" | "blocked" | "cancelled"; originalConsumptionSha256: string | null; attemptId: string | null; jevWorkCredit: 0 }>;
export type ShadowRunAssessment = Readonly<{ schema: "mstar.shadow-assessment/v1"; runId: string; evidenceClass: EvidenceClass; qualification: "component-only" | "synthetic-offline"; w5: false; baselineFrozen: boolean; childEvents: readonly ProbeEvent[]; receipts: readonly WorkUnitReceipt[]; metrics: Readonly<{ childEvents: number; completedUnits: number; blockedUnits: number; cancelledUnits: number; elapsedMs: number; childOutputBytes: number }>; failures: readonly string[] }>;
...
export type WorkUnitDispositionInput = Readonly<{ runId: string; unitId: string; packId: string; packSha256: string; scopeSha256: string; disposition: WorkUnitReceipt["disposition"]; originalConsumption: unknown | null; attemptId?: string | null }>;
export type FrozenShadowEvidence = Readonly<{ baseline: FrozenBaseline; receipts: readonly WorkUnitReceipt[]; childEvents: readonly ProbeEvent[]; evidenceClass: EvidenceClass; elapsedMs: number; childOutputBytes?: number; failures?: readonly string[] }>;
