...
- **Plugin-owned command** — `/mstar-execution` registers its own `ctx.commands` entry (hint `{operation JSON}`) beside that mirror: a closed JSON union of `adopt` (a canonical `exec-session-v1:` reference, adopted only after a read-only C1 session resume succeeds), `clear`, or `run` (a native argv launched shell-free under an overwritten execution identity, with every spoof/legacy env key removed). Identity comes from the carrying native session's own header, known leaf/subagent seats refuse on every operation before any parse or harness probe, and the payload is never steered into the model — a cooperative operator entry, not a lifecycle veto and not an OS process fence (`adopt`/`clear` write only this session's own durable binding slot).
...
- **Record identity** — every NEW row carries a stable `eventId`. Durable workflow events carry the verified source position `{sessionId, streamId, seq}` and the id `wfe1:<kind>:<sessionId>:<streamId>:<seq>`, where `streamId` is the log's VERIFIED native **incarnation** — derived from the durable session header's creation stamp, NEVER the store epoch and never a file mtime — so neither a session cross nor a rebuilt log can collide. Live tool-call rows (`dispatch` / `settle` / `subagent-link` / `workflow-verdict`) carry `wfc1:<sessionId>:<callId>:<kind>` only when the same observation supplies BOTH the carrying session id and a call id; a row without them is recorded WITHOUT an id, never as `wfc1::`. Rows written before this keep their exact bytes and line-offset identity — they simply have no `eventId`.
...
- **Bounded display tail + archived history** — the 500-event display bound is unchanged, but evicted lines are archived byte-exact, `fsync`ed, into sealed chunks under `{HARNESS_DIR}/workflows/<id>/agent-flow-history/chunk-NNNNNN.jsonl` BEFORE the tail is rewritten. The pair of steps is ONE transaction recorded in the transient workflow-relative `agent-flow-compaction.json` journal (`tailBefore` / `tailAfter` full-file sha256, the exact archive range and the removed line count): the record is durable before the archive is touched, the archive before the tail is replaced, and the record is durably removed only once the range is provably complete. Every write path resolves an unfinished transaction first, so no later compaction can pass an unfinished one; a tail matching neither state — or a matching after-state with an incomplete archive range — is refused rather than guessed. Display retention and dedup retention are separate concerns: a row may leave the live tail as soon as its record id is in the durable identity index (below), whatever incarnation it belongs to, because that index — not the tail and not the scan bound — proves the record can never be appended again. Rows without a source keep their bytes and are archived by line position.

### Workflow rows

The same ledger also records **workflow / ralph fan-out runs**: a session-event consumer (logger `mstar/workflow-ledger`, registered at apply) maps the FOUR durable `tool-workflow/*` session events into three new ledger kinds. Source of record is the **durable session events** appended into the CALLING PARENT session's log (top-level runs only — nested transport calls record nothing upstream), **not** the in-memory `workflow/*` emits (roadmap §10.4 N4): the session log is the replayable truth, so the consumer covers it with a **cold scan at apply** (constructor-seeded events never hit the firehose — `firstLiveSeq`) plus a live **`session/event` firehose** listener. The append, the durable identity commit and the scan-bound advance happen in ONE critical section, ordered and each `fsync`ed before the next: ledger row → accepted-identity index entry → cursor.
...
**Dedupe + replay scope.** The dedup authority is the durable **accepted-identity index** — `{HARNESS_DIR}/workflows/<id>/agent-flow-ids.jsonl`, one `fsync`ed line per accepted row — which survives tail compaction and cursor eviction. **One row per record id** across cold+live overlap AND across plugin **re-applies / restarts**: a re-registration consults the persisted index instead of starting with empty state, and an event id already present with DIFFERENT bytes is an advisory refusal (no append, no durable advance) rather than a silent overwrite. The cursor sidecar (`workflow-ledger-cursors.json`, version 2 `{next, stream}` with version-1 read compatibility) is only the per-incarnation **scan bound**; a session whose log head cannot be read records **nothing** rather than inventing an incarnation, and the retired harness-root cursor file is never read as a fallback. A MISSING index is a legitimate first run only while no retained history chunk holds an index-scoped (`wfe1:`) accepted row — a history of only tool-call or legacy rows still proceeds — while an unreadable or damaged index/history, a present-but-unreadable cursor, a failed identity commit and a conflicting reuse of an id all REFUSE instead of being treated as "nothing recorded yet". A session **created after apply** with a constructor-seeded log (resumed / forked conversation — its seeds never publish on the firehose) is cold-scanned **once** on the upstream `session/created` event, and the durable identities keep that backfill idempotent too. A **forked** conversation's scan starts at its `inheritedEventCount`: the inherited prefix is the parent session's history, so the child records only its **own** events — never a second copy of the parent's rows attributed to the child. The cursor sidecar is bounded (per-harness session cap, eviction preferring sessions no longer live); losing it costs a re-walk, never a duplicate row.
...
locale namespace: **"Morning Star Workflow"** (en) / **"晨星工作流"** (zh).
