...
- **Engine single-version pin** — `@mstar-harness/engine` is an exact `2.0.5` devDependency bundled into `dist/` (never a runtime dependency); `readHarnessVersion()` reads the dsh package manifest next to the bundle, which stays `2.0.5` by the single-version invariant.
