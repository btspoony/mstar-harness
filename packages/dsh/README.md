...
When the optional `dsh-llm-fallbacks` capability is **mounted**, a warn-only advisory pass runs **once per apply** (logger `mstar/fallbacks-advisory`): it structurally reads the deployment's fallbacks row config from the loader entry (`options.config` — the same value the fallbacks plugin's `apply()` receives; never the plugin's module internals) and warns, bounded to **at most one warn per category**:

- **(b) missing mstar roles** — mstar role ids absent from the deployment's `roles.list` (one warn listing them). The id set is derived from the `harness-agents/` mirror (shell file stems filtered to `mode: subagent`) — never hardcoded; no mirror → the taxonomy check is skipped with one debug.
- **(c) empty personas** — declared role entities whose `persona` is missing/blank (one warn naming them).
- **(d) legacy keys** — `chains`, `roles.default`, `roles.list[].label`/`.description`, dangling `roles.rules[].role` references, via the applied service's own `detectLegacyKeys` (one warn citing its semantics; skipped — never reimplemented — on the loader-fallback probe path).

A row config that is absent or not an object, or an unreadable `roles.list`, skips the pass with one debug log. The advisory **never writes the fallbacks config** (read-only over the deployment's config layer), never throws, and is **not invoked when fallbacks is unmounted** — it is a signal, not a gate. The pass is attempted at apply and, when the fallbacks row mounts after `dsh` (the loader mounts entries concurrently), runs once at the first `subagent/start` decision point.
