...
- **Self-describing payloads.** Every payload-taking command, regardless of whether it accepts a path through `--file`, another option, or a positional argument, must let a caller construct the payload from the CLI alone: list the required and optional fields with their types in `--help`, or expose a machine-readable schema/example for that payload type. Naming only a TypeScript type (`Absolute CaptureInput JSON path`) is **not** sufficient.
