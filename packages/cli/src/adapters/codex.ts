...
const GLOBAL_ITERATION_SKILLS_WARNING =
  "Codex project-scoped commands (iteration-start / iteration-drive / iteration-loop / codebase-audit / amazing-pr-review / e2e-check) are installed as project-local skills under .agents/skills/ only. Global install skips them to avoid polluting other code agents. Re-run with --scope project to enable.";
...
function ensureIterationSkillLinks(dryRun: boolean) {
  ...
  notes.push(
    "Installed Codex project-scoped command skills under .agents/skills/ (iteration-start, iteration-drive, iteration-loop, codebase-audit, amazing-pr-review, e2e-check) \u2014 symlinked to harness commands/*.md.",
  );
  return notes;
}
