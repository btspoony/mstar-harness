...
4. **Batch** — one session = one PR per **§ Batch sibling PRs**；其余 PR 逐条捕获为 `{HARNESS_DIR}/store.db` 的 open issue（计划内 `mstar plan issue-add`，计划外 `mstar issue add`；capture contract 与 idempotence → **`mstar-project-governance`「Issue capture」**），建议各自独立 session。旧的 backlog 登记动词已退役并拒绝；捕获失败即 `Blocked`，不带着未跟踪的 deferred PR 开审。
