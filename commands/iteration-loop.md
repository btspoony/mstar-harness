...
**Scale budget**（写入 compass；约束 plan 数量）→ **`mstar-iteration`** §1.2：

- **S** → 1 plan
- **M** → 2–3 plans
- **L** → 3–4 plans（默认上限 4，除非 `direction` 显式要求更大）
...
4. Resolve delivery branch policy via **autonomous branch resolve**（`mstar-iteration` §1.2）— never silent `main`/`master`.
...
- [ ] Scale budget applied; plan count within S/M/L
