...
- 已注册的 model-handoff 工具把这些 diagnostics 原样转发进 `not-ready` details，并在拒绝文本中点明类型化原因与下一步受支持操作。diagnostics 只渲染公开 id、代码与规范计划指针：绝不包含信封路径、凭据、session JSON 或环境负载。
