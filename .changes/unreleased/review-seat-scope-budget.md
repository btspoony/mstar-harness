...
- Threaded the rule through the seat side (`qc-specialist-shared`, `reviewer-workflow`, `report-template`, `mstar-sdd` task reviewer) and PM orchestration (`mstar-review-qc`): a truncated seat stops and declares its coverage, and a gate decision on uncovered scope is not an approval. Tightened the residual **severity bar** in `mstar-artifacts` — grade by consequence, not uncertainty; a documentation defect grades by its own consequence when it would drive an unsafe outcome — removing the "when unsure, use `high`" default that turned non-blocking documentation findings into blocking ones.
...
- 将规则贯通到席位侧（`qc-specialist-shared`、`reviewer-workflow`、`report-template`、`mstar-sdd` task reviewer）与 PM 编排（`mstar-review-qc`）：截断的席位停止并声明其覆盖范围，未覆盖范围上的 gate decision 不构成批准。收紧 `mstar-artifacts` 的 residual **severity bar**：按后果而非不确定度分级；文档缺陷若本身会导致不安全后果，则按该后果分级——并移除把非阻断文档发现升级为阻断项的 “when unsure, use `high`” 默认。
