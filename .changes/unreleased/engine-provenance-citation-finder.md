...
- CLI 以 `mstar lint --type provenance <target>` 开放该扫描：显式内容类型（推断永不选中），对目标文件或目录遍历的校准文本面（`.md`/`.ts`——`README.md` 等普通命名文档同样收集）运行发现器；退出码语义不变（1 = 存在引用，2 = 用法错误）。`docs/cli.md` 已登记新 `--type` 取值。
