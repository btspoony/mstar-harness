...
- Tightened the store's open path while it was being fixed: a journal is only restored for a file whose complete SQLite header says WAL (format magic, legal page size, payload fractions, schema/text-encoding values, page-aligned size) — never for bytes that merely look like one — and the store path is discriminated without following links, before the driver opens it and before any "absent" verdict: a symlinked, dangling or otherwise non-regular store path is an unreadable store refused `store.corrupt`, so no store intent follows a link out of the resolved control root and the retired file route cannot answer for it.
...
- 顺带收紧 store 打开路径：只有完整 SQLite 文件头表明是 WAL（格式魔数、合法页大小、payload 常量、schema/文本编码值、页对齐的文件大小）时才补回日志——看起来像的字节不补；store 路径的判别不跟随链接，先于驱动打开、也先于任何「不存在」判定：符号链接（含悬空）或非普通文件的 store 路径按不可读 store 以 `store.corrupt` 拒绝，任何 store 意图都不会跟随链接离开解析后的控制根，退役的文件路线也无法为其作答。
