...
- `mstar persist get --versioned` returns `{payload,version}` (the `sha256:` byte version, or the `absent` token) and refuses a pluggable store module; a bare `put`/`delete` of a protected kind is refused before anything is written (the replacement face is documented in the coordinated-replacement fragment).
...
- `mstar persist get --versioned` 返回 `{payload,version}`（`sha256:` 字节版本，或 `absent` 令牌），并拒绝可插拔 store module；受保护类型的裸 `put`/`delete` 会在写入任何内容之前被拒绝（替换面见协调替换片段）。
