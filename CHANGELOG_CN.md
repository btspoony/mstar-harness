...
- 清理面向用户 skills 中的本地维护路径引用（`.mstar/iterations/…`、`.harness/docs/…`）：`mstar-host/references/dsh.md` 与 `mstar-iteration/references/phase-2-worktree-lease.md` 现仅引用运行时消费者契约（复核零残留）。
