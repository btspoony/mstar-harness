...
- **可插拔 JSON 持久化** —— 协调文档（`status.json`、workflow snapshots、project residuals）经 `ArtifactStore` 持久化；默认 `FsStore` 保持既有 `.mstar/` 路径，集成方可经 `MSTAR_STORE_MODULE` / `--store` / 进程内 `setArtifactStore` 挂载自有存储
