...
| `/pr-deep-review [pr\|branch\|scope]` | 合并前对 PR / 分支 / diff 做深度审查 → 给出唯一结论（`ship it` / `needs fixes` / `blocked`）与全部发现；有 PR 编号时自动发布 GitHub Review。单个 PR 走三阶段流水线（collect → domain review → main-agent synthesis；one verdict / one GitHub Review）。多 PR 输入 → 仅审查第一个 PR；其余 PR 登记为审计待办（下一次会话）；建议一个会话只审一个 PR。 |
