...
| `/pr-deep-review [pr\|branch\|scope] [full]` | 合并前对 PR / 分支 / diff 做证据先行的深度审查 → 单一结论（`ship it` / `needs review` / `blocked`）。<br>worktree 隔离、只读；发现可转为 plan 进入 Prepare → Execute。<br>有 PR 编号时强制发布 GitHub Review（在发现对应的代码行上评论）。<br>`full` — 展示全部发现（默认只列 top 1–3）。 |
